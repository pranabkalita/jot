<template>
    <div>
        <div v-if="loading">Loading ...</div>
        <div v-else>
            <div class="flex justify-between">
                <a href="#" class="text-blue-400" @click="$router.back()">
                    < Back
                </a>

                <div class="relative">

                    <router-link
                        :to="'/contacts/' + contact.contact_id + '/edit'"
                        class="px-4 py-2 rounded text-green-500 border border-green-500 text-sm font-bold mr-2"
                    >
                        Edit
                    </router-link>

                    <a
                        href="#"
                        class="px-4 py-2 border border-red-500 rounded text-sm font-bold text-red-500"
                        @click="toggleDeleteModal"
                    >
                        Delete
                    </a>

                    <div v-if="isDeleting" class="absolute bg-blue-900 text-white rounded-lg z-20 p-8 w-64 right-0 mt-4">
                        <p>Are you sure you want to delete this record?</p>

                        <div class="flex items-center mt-6 justify-end">

                            <button class="text-white pr-4" @click="toggleDeleteModal">Cancel</button>

                            <button
                                class="px-4 py-2 bg-red-500 rounded text-sm font-bold text-white"
                                @click="deleteContact"
                            >
                                Delete
                            </button>

                        </div>
                    </div>
                </div>

                <div
                    v-if="isDeleting"
                    class="bg-black opacity-25 absolute right-0 left-0 top-0 bottom-0 z-10"
                    @click="toggleDeleteModal">
                </div>
            </div>

            <div class="flex items-center pt-6">
                <user-circle :name="contact.name" />

                <p class="pl-5 text-xl">{{ contact.name }}</p>
            </div>

            <p class="pt-6 text-gray-600 font-bold uppercase text-sm">Contact</p>
            <p class="pt-2 text-blue-400">{{ contact.email }}</p>

            <p class="pt-6 text-gray-600 font-bold uppercase text-sm">Company</p>
            <p class="pt-2 text-blue-400">{{ contact.company }}</p>

            <p class="pt-6 text-gray-600 font-bold uppercase text-sm">Birthday</p>
            <p class="pt-2 text-blue-400">{{ contact.birthday }}</p>
        </div>

    </div>
</template>

<script>
import { mapActions } from 'vuex';
import UserCircle from '../Components/UserCircle.vue';


export default {
    name: 'ContactsShow',

    components: {
        UserCircle,
    },

    data: () => ({
        contact: null,
        loading: false,
        isDeleting: false,
    }),

    methods: {
        ...mapActions(['getContact', 'destroyContact']),

        toggleDeleteModal: function() {
            this.isDeleting = !this.isDeleting;
        },

        deleteContact: async function() {
            try {
                await this.destroyContact(this.$route.params.id);

                this.$router.push('/contacts');
            } catch (error) {
                this.$router.push('/contacts');
            }
        },
    },

    async created() {
        this.loading = true;
        try {
            this.contact = await this.getContact(this.$route.params.id);

            this.loading = false;
        } catch (error) {
            this.loading = false;

            this.$router.push('/contacts');
        }
    }
}
</script>
