    ...
    <div class="h-screen bg-white">
        <div class="flex">
            <div class="pl-6 bg-gray-200 w-48 h-screen border-r-2 border-gray-400">
                <nav class="pt-4">
                    <router-link to="/">
                        <svg class="fill-current text-blue-600 w-12 h-12 w-16" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
...
<script>
import UserCircle from './views/Components/UserCircle.vue';
...
export default {
    ...
    created() {
        ...
        window.axios.interceptors.request.use((config) => {
            config.headers = { ...config.headers, 'Authorization': `Bearer ${this.user.api_token}` };
